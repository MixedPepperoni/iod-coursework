...
function ShoppingList({ selectedItems = [], sessionId }) {
  console.log("Fetching items for sessionId:", sessionId);
  ...
  // Fetch items? or do it when sessionId changes?
  useEffect(() => {
    if (!sessionId) return; // guards if sessionId isnt define, reduce breakage.
    setLoading(true);
    setError(null);
  ...
  const handleAddItems = async () => {
    ...
    try {
      ...
      setItems((prev) => [
        ...prev,
        { ...savedItem, quantity, key: savedItem._id },
      ]);
      setName("");
      setQuantity(1);
      setPrice(0);
